 ...
 *
 * Usage:
 *   bun otel-receiver.ts                           # all modes with defaults
 *   bun otel-receiver.ts --http --unix --file      # all modes explicitly
 *   bun otel-receiver.ts --http --port 4318        # HTTP only
 *   bun otel-receiver.ts --unix --sock /tmp/o.sock # Unix only
 *   bun otel-receiver.ts --file --dir ~/telemetry  # File only
 */
...
function formatSpan(span: OTLPSpan, indent = 0): void {
	...
	if (isMain) {
		...
		console.log();
		console.log(`${c.bold}${c.cyan}━━━ MAIN SPAN ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${c.reset}`);
