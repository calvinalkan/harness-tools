...
import { createEditTool, createWriteTool, type ExtensionAPI } from "@mariozechner/pi-coding-agent";
...
export default function agentSandboxExtension(pi: ExtensionAPI): void {
 ...
 */
function registerWriteEditHandlers(pi: ExtensionAPI, cwd: string): void {
	// Override write tool
			...
			try {
				...
				if (isFileNotFoundError(message)) {
					...
					if (fileExists(path, cwd)) {
						ctx.ui.notify("─── Edit blocked by sandbox ───", "error");
						throw new Error(
							`Cannot edit file: ${path} (file exists but is protected by sandbox)${SANDBOX_EDIT_HINT}`,
							{ cause: err },
						);
					}
					throw err;
