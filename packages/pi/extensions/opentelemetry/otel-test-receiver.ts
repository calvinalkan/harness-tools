...
function formatSpan(span: OTLPSpan, indent = 0): void {
  ...
  const isError =
    span.status?.code === 2 || getAttr(span, "tool.is_error") === true;

  const statusIcon = isError ? `${c.red}✗${c.reset}` : `${c.green}✓${c.reset}`;

  if (isMain) {
    // Main span - prominent header
    console.log();
    console.log(
      `${c.bold}${c.cyan}━━━ MAIN SPAN ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${c.reset}`,
    );
    console.log(
      `${c.gray}${time}${c.reset} ${statusIcon} ${c.bold}${duration}${c.reset}`,
    );
    ...
    if (inputText) {
      ...
      console.log(
        `${pad}  ${c.dim}input:${c.reset} "${truncated}${String(inputText).length > 60 ? "..." : ""}"`,
      );
    }
    ...
    if (cost !== undefined) {
      console.log(
        `${pad}  ${c.dim}cost:${c.reset} $${Number(cost).toFixed(4)}`,
      );
    }
  } else if (span.name.startsWith("turn-")) {
    // Turn span
    ...
    console.log(
      `${pad}${c.dim}│${c.reset}   ${c.magenta}🔧 ${toolName}${c.reset}${detail} ${c.dim}(${duration})${c.reset} ${statusIcon}`,
    );
  } else {
    // Unknown span type
    console.log(
      `${pad}${c.blue}[${span.name}]${c.reset} ${duration} ${statusIcon}`,
    );
  }
}
...
async function main(): Promise<void> {
  ...
  const enableHttp =
    values.http || (!values.http && !values.unix && !values.file);
  const enableUnix =
    values.unix || (!values.http && !values.unix && !values.file);
  const enableFile =
    values.file || (!values.http && !values.unix && !values.file);
